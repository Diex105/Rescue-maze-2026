...
SENAL_SILENCE_MS = 4000 #3500

ultima_letra_enviada = None
letras_cooldown = {}
COOLDOWN_LETRA_MS = 12000  # 8 segundos
...
#MODELO TARGET
net_target = ml.Model("targets.tflite", load_to_fb=True)
labels_target = [line.rstrip('\n') for line in open("labelsTAARGET.txt")]

min_conf_target = 0.7
threshold_list_target = [(math.ceil(min_conf_target * 255), 255)]
...
def fomo_post_process_target(model, inputs, outputs):
    ob, oh, ow, oc = model.output_shape[0]
    ...
    for i in range(oc):
        ...
        for b in blobs:
            ...
            score = img_channel.get_statistics(
                thresholds=threshold_list_target,  # 🔥 AQUÍ TAMBIÉN
                roi=rect
            ).l_mean() / 255.0
...
while True:
    ...
    for i, detection_list in enumerate(net_target.predict([img], callback=fomo_post_process_target)):
        if i == 0:
            continue
        ...
        for x, y, w, h, score in detection_list:
            if score > 0.5:
                ...
                print("TARGET:", labels_target[i])
                img.draw_circle((cx, cy, 12), color=(0,255,0))

    if target_detectado:

        img.draw_string(5, 5, "TARGET ACTIVO")

        if not senal_is_silent():

            poner_voltaje_letra("omega")

            pin_aviso.high()
            time.sleep_ms(100)
            pin_aviso.low()

            print("TARGET -> OMEGA (SIN CIRCULO)")
