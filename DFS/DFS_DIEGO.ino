...
void avanzarInclinacion(float anguloObjetivo) {
  ...
  while (true) {
    ...
    if (abs(roll) > 60 || abs(pitch) > 60) {
      ...
      int recuperado = 0;
      while (true){
        sensors_event_t eventRec;
        bno.getEvent(&eventRec);

        float pitchRec = eventRec.orientation.z;
        float rollRec  = eventRec.orientation.y;

        if(abs(rollRec) < 20 && abs(pitchRec) < 20){
          recuperado++;
        }else{
          recuperado = 0;
        }
...
void parpadearNeopixel(int veces, uint8_t r, uint8_t g, uint8_t b, int delayMs = 120) {
  if (veces <= 0) {
  ...
  for (int bidx = 0; bidx < veces; bidx++) {
    for (int i = 0; i < NUMPIXELS; i++) {
...
int tirarNKits(int cantidad, uint8_t r, uint8_t g, uint8_t b, int parpadeosPorKit) {
  int lanzados = 0;
  ...
  for (int i = 0; i < cantidad; i++) {
    ...
    if (contadorKitsDer < MAX_KITS_POR_SERVO) {
      parpadearNeopixel(parpadeosPorKit, r, g, b, 100);
      accionarServo(SR, SERVO_DER_CERRADO, SERVO_DER_EMPUJE);
