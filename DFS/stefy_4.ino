...
// ===============================
#define BASE_SPEED 180//180
#define TURN_SPEED_MAX 200//200
...
void leerTOFS() {
  ...
  const uint16_t DIST_LIBRE = 8190;   // valor artificial para "sin pared"
  const uint16_t MIN_UTIL   = 25;     // mínimo creíble
  const uint16_t MAX_UTIL   = 1200;   // máximo útil para tu aplicación
  ...
  for (int i = 0; i < NUM_SENSORES; i++) {
    ...
    // Si la lectura no sirve, NO conservar la anterior
    if (status == 4 || lectura == 0 || lectura < MIN_UTIL || lectura > MAX_UTIL) {
      Dist[i] = DIST_LIBRE;
    } 
    else {
      int d = lectura;
