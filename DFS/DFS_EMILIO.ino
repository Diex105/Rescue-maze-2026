...
const int SERVO_DER_CERRADO = 20;
const int SERVO_DER_EMPUJE  = 150;
//comunicación
...
/*void turnToHeading(float objetivo) {
  ...
  const int VEL_MIN = 140;
  const int VEL_FINE = 105;
  ...
  const unsigned long TIMEOUT_TURN = 7000;   // 5 segundos maximo
  unsigned long tInicioTurn = millis();
  bool timeoutTurn = false;
  ...
  while (true) {
    ...
    // =========================
    // TIMEOUT GLOBAL DEL GIRO
    // =========================
    if (millis() - tInicioTurn > TIMEOUT_TURN) {
      Serial.println("Timeout turnToHeading");
      timeoutTurn = true;
      stopMotors();
      break;
    }
    ...
    // Detectar oscilacion cerca del objetivo
...
void turnToHeading(float objetivo) {
  ...
  const int VEL_MIN = 170;        // antes 150, demasiado alta
  const int VEL_FINE = 155;       // velocidad fija para corrección fina
...
int tirarNKits(int cantidad) {
  ...
  leerValor = false;
  valorPendiente = -1;
  bloquearOpenMV_hasta = millis() + 2000;
...
bool procesarSenalOpenMV() {
  ...
  int valor = decodificarValorOpenMV(v);
...
bool atenderDeteccionOpenMV() {
  ...
  if (Dist[4] == 20 || Dist[4] > UMBRAL_PARED_DERECHA) {
  ...
  while (millis() - t0 < 5000) {
    ...
    if (!kitsYaLanzados) {
      ...
      if (kitsSolicitados > 0) {
        lanzados = tirarNKits(kitsSolicitados);
      }
