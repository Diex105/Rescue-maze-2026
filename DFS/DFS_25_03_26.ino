...
int n = 20;
volatile int colorBlanco[4] = {0, 0, 0, 0};
const int NUM_PLATEADOS = 4;
volatile int colorPlateado[NUM_PLATEADOS][4];
...
bool revisarUART(float anguloObjetivo) {
  ...
  const unsigned long tiempoBloqueoMismoDato = 300;
  ...
  while (openmv.available()) {
    ...
    if (c == '\n') {
      ...
      if (dato == ultimoDatoProcesado &&
          (millis() - ultimoTiempoDato) < tiempoBloqueoMismoDato) {
    ...
    // Solo guardar caracteres imprimibles
    if (isPrintable(c)) {
      buffer += c;

      // Protección por si llega basura infinita
      if (buffer.length() > 30) {
        buffer = "";
      }
    } else {
      // si llega basura rara, reinicia buffer
      buffer = "";
    }
  }
...
long distanciaColor(uint16_t r, uint16_t g, uint16_t b, uint16_t c, volatile int ref[4]) {
  return
    abs((int)r - ref[0]) +
    abs((int)g - ref[1]) +
    abs((int)b - ref[2]) +
...
String detectarColor(){
  ...
  // margen para evitar empates falsos
  if (diferencia > 80) {
...
void calibrarReferenciaColor(volatile int destino[4], const char* nombreSuperficie) {
...
void setup() {
  ...
  Serial.println("Coloca el robot sobre BLANCO");
  delay(4000);
  ...
  for (int i = 0; i < NUM_PLATEADOS; i++) {
    mostrarOLED("PLATEADO", "MUESTRA " + String(i+1));
    delay(5000);
    calibrarReferenciaColor(colorPlateado[i], "PLATEADO");
    mostrarOLED("SIGUIENTE...");
    delay(2000);
  }

  mostrarOLED("REGRESA A", "BLANCO");
  Serial.println("Regresa el robot a BLANCO");
  delay(5000);


  mostrarOLED("LISTO");
  Serial.println("Calibracion terminada. Iniciando robot...");
  delay(1000);
