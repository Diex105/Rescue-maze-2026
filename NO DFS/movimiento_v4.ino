...
void avanzar_optimizado(int DISTANCIA_CM) {
  ...
  while (true) {
    ...
    if (deltaTime >= 0.01) { // Control cada 10ms
      ...
      bool paredIzq = Dist[1] < 250 || Dist[3] < 250;
      bool paredDer = Dist[4] < 250 || Dist[6] < 250;
      float correccionLat = 0;
      if (paredIzq && paredDer) {
        correccionLat = calibrar_lateral();
      }

      float correccionTotal = correccion + correccionLat *0.5;
