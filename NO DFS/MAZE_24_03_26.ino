...
#define TURN_SPEED_MAX 255
#define TURN_SPEED_MIN 210
...
ResultadoMovimiento avanzar_optimizado(int DISTANCIA_CM) {
  ...
  const float KP = 1.4;      // Reducido para movimiento más suave
  ...
  while (true) {
    ...
    static int contadorSalirPlateado = 0;
    if (ignorarPlateado) {
      if (color != "PLATEADO") {
        contadorSalirPlateado++;
      } else {
        contadorSalirPlateado = 0;
      }
      if (contadorSalirPlateado >= 3) {
...
bool revisarUART(float anguloObjetivo) {
  ...
  const unsigned long tiempoBloqueoMismoDato = 0;
...
float controlLateralContinuo() {
  ...
  const float KP_LAT = 0.9;
  const float MIN_PARED = 40.0;
  const float MAX_PARED = 200.0;
  const float UMBRAL = 2.0;
  ...
  if (paredIzq && paredDer) {
    ...
    const float DISTANCIA_OBJ = 109.0;  // Ajustable
    correccion = (DISTANCIA_OBJ - s1) * KP_LAT;
  }

 
  else if (paredDer) {
    const float DISTANCIA_OBJ = 111.0;  // Ajustable
...
String detectarColor(){
  ...
  } else if (c >= 230 && c <= 800 && totales < 35) {
...
void cargarPrioridadDFS() {
  prioridadDirs[0] = girarDerechaDir(dirActual);
  prioridadDirs[1] = dirActual;
  prioridadDirs[2] = girarIzquierdaDir(dirActual);
  prioridadDirs[3] = opuestaDir(dirActual);
}
...
bool obtenerDireccionNueva(Direccion &dElegida) {
  ...
  for (int i = 0; i < 4; i++) {
    Direccion d = (Direccion)prioridadDirs[i];
