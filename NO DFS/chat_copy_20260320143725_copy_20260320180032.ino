...
#define TURN_SPEED_MAX 220
#define TURN_SPEED_MIN 180
#define TICKS_POR_CM 225.0

Encoder encLeft(15, 14);
Encoder encRight(11, 12);

const int SERVO_IZQ_CERRADO = 170;
const int SERVO_IZQ_EMPUJE  = 40;

const int SERVO_DER_CERRADO = 40;
const int SERVO_DER_EMPUJE  = 140;
...
uint8_t direcciones[NUM_SENSORES] = {0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x3A};
...
void turnToHeading(float objetivo) {
  ...
  const int VEL_MIN = 115;
  const float ANGULO_TOL = 0.8;
...
void giroRelativo(float grados) {
  if (!bnoActivo) return;
  float inicio = readHeadingAvg(10);
  float objetivo = normalize360(inicio + grados);

  display.clearDisplay();
  display.setCursor(5, 5);
  display.print("Giro ");
  display.print(grados, 1);
  display.display();
...
void giro90Der() {
  // aunque se llame "Der", lo forzamos a girar a la izquierda
  giro45Izq();
  giro45Izq();
}

void revisarVictimaEnGiro() {
  float headingActual = readHeadingAvg(3);
  unsigned long t0 = millis();
  while (millis() - t0 < 180) {
    revisarUART(headingActual);
    delay(10);
  }
}
...
void giro180() {
  for (int i = 0; i < 4; i++) {
    ...
    stopMotors();
    delay(70);
    revisarVictimaEnGiro();
  }
}

void giro45Izq() { giroRelativo(45.0); }
void giro45Izq2() { giroRelativo(-45.0); }
void giro35Izq() { giroRelativo(-35.0); }
void giro35Der() { giroRelativo(35.0); } 
...
void leerTOFS() {
  ...
  for (int i = 0; i < NUM_SENSORES; i++) {
    ...
    if (i == 6) Dist[6] = Dist[6] - 15;
    if (i == 5) Dist[5] = Dist[5] - 28;
    if (i == 2) Dist[2] = Dist[2] + 8;
...
void retrocederRecto(float distanciaCM, float anguloObjetivo) {
  long ticksObjetivo = distanciaCM * TICKS_POR_CM;
  encLeft.write(0);
  encRight.write(0);
  ...
  while (true) {
    ...
    float dError = (error - lastError) / dt;
    float correccion = 0.8 * error + 0.03 * dError;

    int leftSpeed = constrain(BASE_SPEED - correccion, 120, 220);
    int rightSpeed = constrain(BASE_SPEED + correccion, 120, 220);

    backwardRaw(leftSpeed, rightSpeed);
...
String detectarColor() {
  ...
  if (r < (colores[0] * 0.2) && g < (colores[1] * 0.15) && b < (colores[2] * 0.15) && c < (colores[3] * 0.2)) {
    return "NEGRO";
  } else if (r < (colores[0] * 0.2) && g < (colores[1] * 0.45) && b < (colores[2] * 0.45) && c > (colores[3] * 0.2)) {
    return "AZUL";
  } else if (r < (colores[0] * 0.9) && g < (colores[1] * 0.3) && b > (colores[2] * 0.08) && c > (colores[3] * 0.2)) {
    return "ROJO";
...
bool revisarUART(float anguloObjetivo) {
  static String ultimoDatoProcesado = "";
  static unsigned long ultimoTiempoDato = 0;
  const unsigned long tiempoBloqueoMismoDato = 3000;
  ...
  if (dato == "L:omega") kits = 0;
    ...
    if (dato == String(valor)) {
      if (valor < 0) return false;
      if (valor == 0) kits = 0;
      else if (valor == 1) kits = 1;
      else if (valor == 2) kits = 2;
    }
  }

  if (kits == -1) return false;

  // Bloquear solo si es EXACTAMENTE el mismo dato en poco tiempo
  if (dato == ultimoDatoProcesado && (millis() - ultimoTiempoDato) < tiempoBloqueoMismoDato) {
    Serial.println("UART repetido bloqueado");
    return false;
  }
  ...
  pixels.show();
  delay(250);  // antes 1000, muy lento
  ...
  for (int k = 0; k < kits; k++) {
    if (contadorKitsTotal >= 8) break;

    if (contadorKitsTotal < 4) accionarServo(SL);
    else accionarServo(SR);
  ...
  ultimoDatoProcesado = dato;
  ultimoTiempoDato = millis();
...
float controlLateralContinuo() {
  ...
  if (paredIzq && paredDer) {
    ...
    if (abs(diferencia) > UMBRAL) correccion = diferencia * KP_LAT;
  } else if (paredIzq) {
...
void esperarEstable() {
  ...
  while (estables < 4) {
    ...
    if (abs(angleDiff(actual, prev)) < 0.2) estables++;
    else estables = 0;
    prev = actual;
  }
} 

void acomodarObstaculoSimple(String lado, float distanciaAvanzada) {
  const float DISTANCIA_TILE = 30.0;
  const float RETROCESO_CM_LOCAL = 6.0;
  ...
  float anguloOriginal = readHeadingAvg(1);

  stopMotors();
  delay(60);
  retrocederRecto(RETROCESO_CM_LOCAL, anguloOriginal);

  float objetivoDesvio;
  float avanceDiagonal;

  if (lado == "DER") {
    objetivoDesvio = normalize360(anguloOriginal - ANGULO_DESVIO_DER);
    avanceDiagonal = AVANCE_DIAGONAL_DER;
  } else {
    objetivoDesvio = normalize360(anguloOriginal + ANGULO_DESVIO_IZQ);
  ...
  float distanciaPendiente = DISTANCIA_TILE - distanciaAvanzada;
  distanciaPendiente -= RETROCESO_CM_LOCAL;
...
bool libreFrente() { return Dist[0] > UMBRAL_PARED; }
bool libreIzquierda() { return Dist[1] > UMBRAL_PARED; }
bool libreDerecha() { return Dist[4] > UMBRAL_PARED; }
...
void decisionDFS() {
  ...
  if (derecha) {
    ...
    Serial.println("DFS: Giro Izquierda");
    giro90Izq();
  } else {
...
void avanzar_optimizado(int DISTANCIA_CM) {
  ...
  while (true) {
    ...
    int pwmEstimado = max(abs(currentLeftSpeed), abs(currentRightSpeed));
    ...
    if (enContacto) {
      ...
      stopMotors();
      delay(40);
      retrocederRecto(RETROCEDER_CM, anguloObjetivo);
      turnToHeading(anguloObjetivo);
      encLeft.write(0);
      encRight.write(0);
      enContacto = false;
    ...
    if (R || L) {
      ...
      if (R && !L) acomodarObstaculoSimple("DER", distanciaChoque);
      else if (L && !R) acomodarObstaculoSimple("IZQ", distanciaChoque);
      else acomodarObstaculoSimple("DER", distanciaChoque);
...
void iniciarTOFS() {
  ...
  for (int i = 0; i < NUM_SENSORES; i++) {
    ...
    if (!sensores[i].begin(0x29)) sensorActivo[i] = false;
    else {
...
void setup() {
  ...
  SR.attach(24);
  SL.attach(25);
  SL.write(SERVO_IZQ_CERRADO);
  SR.write(SERVO_DER_CERRADO);
  delay(700);
