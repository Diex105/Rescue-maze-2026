...
void avanzar_optimizado(int DISTANCIA_CM) {
  ...
  while (true) {
    ...
    if (deltaTime >= 0.01) { // Control cada 10ms
      ...
      // 4. Aplicar corrección a las velocidades
      // Si el error es positivo, debe girar hacia un lado; si es negativo, al otro.
      int targetLeft = BASE_SPEED + (int)correccion;
      int targetRight = BASE_SPEED - (int)correccion;
