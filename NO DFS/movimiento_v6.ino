...
void calibrar_atras() {
  ...
  // Solo ejecutar si hay pared atrás y NO hay tres paredes
  if (!paredAtras || tresParedes) return;

  float atras = (Dist[7] + Dist[8]) / 2.0;

  // Si ya está bien alineado no hacer nada
  ...
  while (true) {
    ...
    float atras = (Dist[7] + Dist[8]) / 2.0;
    float error = DIST_OBJ - atras;

    if (abs(error) <= UMBRAL) break;

    int vel = VEL_BASE + abs(error * KP_BACK);
    vel = constrain(vel, 30, 80);
