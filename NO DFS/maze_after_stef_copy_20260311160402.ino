...
/*loat calibrar_lateral() {

  const float KP_LAT = 0.35;
  const float KP_LAT_SINGLE = 0.15;
  const float KP_ANG = 0.15;

  const float MIN_PARED = 40.0;
  const float MAX_PARED = 200.0;

  const float UMBRAL = 5.0;
  const float DIF_OBJ = 11.5;
  ...
  float correccionDist = 0;
  float correccionAng = 0;
  ...
  if (paredIzq && paredDer) {
    ...
    if (abs(errorCentro) > UMBRAL) {
      correccionDist = errorCentro * KP_LAT;
    }

    float angIzq = s1 - s3;
    float angDer = s4 - s6;

    correccionAng = (angIzq - angDer) * KP_ANG;
  }
  ...
  // -----------------------
    ...
    if (abs(errorCentro) > UMBRAL) {
      correccionDist = errorCentro * KP_LAT_SINGLE;
    }

    float angIzq = s1 - s3;
    correccionAng = angIzq * KP_ANG;
  }
  ...
  // -----------------------
    ...
    const float DIST_OBJ_DER = 124.0;
    ...
    float angDer = s4 - s6;
    correccionAng = -angDer * KP_ANG;
  }

  float correccion = correccionDist + correccionAng;

  correccion = constrain(correccion, -20, 20);
...
void calibrar_lateral() {
  ...
  while (true) {
    ...
    if (modoLateral == 1){
      ...
      const float DIST_OBJ_DER = 140.0;
...
String detectarColor(){  float anguloObjetivo = readHeadingAvg(5); 
  uint16_t r, g, b, c;
...
void reaccionColor(String color, float distanciaRecorrida) {
  ...
  if (color == "PLATEADO") {
    display.clearDisplay();
    display.setCursor(0,0);
    display.println("Roier el chido");
    display.display();
  }
}
// =============================== INICIAR SENSORES
