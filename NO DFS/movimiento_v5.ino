...
void avanzar_optimizado(int DISTANCIA_CM) {
  ...
  while (true) {
    ...
    if (deltaTime >= 0.01) { // Control cada 10ms
      ...
      float correccionTotal = (correccion * 0.3) + (correccionLat * 0.7);
...
void giro180() {
  ...
  while (abs(error) > 1.5) {
    ...
    actual = readHeadingAvg(5);
    error = angleDiff(objetivo, actual);
  }
...
/*float calibrar_lateral() {
  ...
  if (paredIzq && paredDer) {
    ...
    correccion = (DISTANCIA_OBJ - izquierda) * KP_LAT;
  }

 
  else if (paredDer) {
    const float DISTANCIA_OBJ = 120.0;  // Ajustable
    correccion = (derecha - DISTANCIA_OBJ) * KP_LAT;
  }
...
bool revisarUART(float anguloObjetivo) {
  ...
  if (dato == "L:omega" || dato == "L:psi" || dato == "L:phi") {
...
float controlBNO() {
  ...
  const float KP_BNO = 1.8;

  float headingActual = readHeadingAvg(3);

  float error = headingObjetivo - headingActual;
...
/*float controlLateralContinuo() {
  ...
  const float DIST_OBJ_DER = 120.0;
  const float DIST_OBJ_IZQ = 120.0;

  float s1 = Dist[1];   // izquierda
  float s4 = Dist[4];   // derecha
...
/*float controlLateralContinuo() {
  ...
  const float KP_LAT = 0.25;        // ganancia lateral más suave
  const float MIN_PARED = 40.0;
  const float MAX_PARED = 400.0;
  const float UMBRAL = 3.0;         // umbral para evitar correcciones por ruido
  const float HISTERESIS = 5.0;     // mm de histéresis para entrada/salida
  const float MAX_CORR = 60.0;      // límite de corrección en ticks (o escala que uses)

  // Distancias objetivo basadas en tus lecturas
  const float DIST_OBJ_IZQ = 106.0; // mm (promedio de s1+s3)
  const float DIST_OBJ_DER = 109.0; // mm (promedio de s4+s6)

  // Lecturas crudas de los TOF
  float raw_sIzq1 = Dist[1];
  float raw_sIzq3 = Dist[3];
  float raw_sDer4 = Dist[4];
  float raw_sDer6 = Dist[6];

  // Filtrado median de 3 para cada sensor (protege contra picos)
  float sIzq1 = median3(raw_sIzq1, raw_sIzq1, raw_sIzq1); // si no guardas histórico, median3 con mismas lecturas es neutro
  float sIzq3 = median3(raw_sIzq3, raw_sIzq3, raw_sIzq3);
  float sDer4 = median3(raw_sDer4, raw_sDer4, raw_sDer4);
  float sDer6 = median3(raw_sDer6, raw_sDer6, raw_sDer6);

  // Promedio por lado (usar solo sensores válidos)
  float sIzq = 1000.0;
  float sDer = 1000.0;
  int cntIzq = 0, cntDer = 0;

  if (sIzq1 > MIN_PARED && sIzq1 < MAX_PARED) { sIzq += 0; sIzq = (sIzq1 + 0) - 0; cntIzq++; } // placeholder para evitar warnings
  // mejor forma: sumar condicionalmente
  sIzq = 0.0;
  if (sIzq1 > MIN_PARED && sIzq1 < MAX_PARED) { sIzq += sIzq1; cntIzq++; }
  if (sIzq3 > MIN_PARED && sIzq3 < MAX_PARED) { sIzq += sIzq3; cntIzq++; }
  if (cntIzq > 0) sIzq = sIzq / cntIzq;
  else sIzq = 1000.0;

  sDer = 0.0;
  if (sDer4 > MIN_PARED && sDer4 < MAX_PARED) { sDer += sDer4; cntDer++; }
  if (sDer6 > MIN_PARED && sDer6 < MAX_PARED) { sDer += sDer6; cntDer++; }
  if (cntDer > 0) sDer = sDer / cntDer;
  else sDer = 1000.0;

  bool paredIzq = (sIzq > MIN_PARED && sIzq < MAX_PARED);
  bool paredDer = (sDer > MIN_PARED && sDer < MAX_PARED);

  float correccion = 0.0;

  // Si hay pared en ambos lados → centrar respecto a los objetivos
  if (paredIzq && paredDer) {
    // error relativo a objetivos
    float errIzq = DIST_OBJ_IZQ - sIzq;
    float errDer = sDer - DIST_OBJ_DER;

    // Si ambos errores pequeños, no corregir
    if (abs(errDer - errIzq) <= UMBRAL) {
      correccion = 0.0;
    } else {
     
      float objetivoMedio = (DIST_OBJ_IZQ + DIST_OBJ_DER) / 2.0;
      float actualMedio = (sIzq + sDer) / 2.0;
      float errMedio = objetivoMedio - actualMedio;
      correccion = errMedio * KP_LAT;
    }
  }

  // Solo pared izquierda: acercarse/alejar para alcanzar DIST_OBJ_IZQ
  else if (paredIzq) {
    float error = DIST_OBJ_IZQ - sIzq;
    if (abs(error) > UMBRAL) correccion = error * KP_LAT;
  }

  // Solo pared derecha: acercarse/alejar para alcanzar DIST_OBJ_DER
  else if (paredDer) {
    float error = sDer - DIST_OBJ_DER;
    if (abs(error) > UMBRAL) correccion = error * KP_LAT;
  }

  // Limitar corrección para evitar cambios bruscos
...
float controlLateralContinuo() {
  ...
  const float KP_LAT = 0.3;
  const float MIN_PARED = 40.0;
  const float MAX_PARED = 400.0;
  const float UMBRAL = 5.0;

  const float DIST_OBJ_DER = 106.0;
  const float DIST_OBJ_IZQ = 109.0;

  // Lecturas de los TOF
  float sIzq1 = Dist[1];   // izquierda frontal
  float sIzq3 = Dist[3];   // izquierda trasera
  float sDer4 = Dist[4];   // derecha frontal
  float sDer6 = Dist[6];   // derecha trasera

  // Promedio de cada lado
  float sIzq = (sIzq1 + sIzq3) / 2.0;
  float sDer = (sDer4 + sDer6) / 2.0;
...
void calibrar_lateral() {   //Version diego
  ...
  const float KP_LAT = 0.3;   // un poco más suave
  ...
  while (true) {
    ...
    const float UMBRAL_CENTRADO = 8.0;
    ...
    float correccionAng = errorAng * 1.0;
...
/*void avanzar_con_paredes() {

  const int VEL_BASE = 55;

  const float KP_LAT = 0.35;
  const float KP_ANG = 0.45;

  const float MIN_PARED = 40;
  const float MAX_PARED = 400;

  const float DIF_OBJ = 11.5;   // diferencia real cuando está centrado
  ...
  while (true) {
    ...
    // mantener misma diferencia lateral
    if (paredIzq && paredDer) {
      ...
      const float DIST_OBJ_IZQ = 112.5;
      float error = DIST_OBJ_IZQ - izquierda;
      correccion += error * KP_LAT;
      ...
      const float DIST_OBJ_DER = 124;
      float error = derecha - DIST_OBJ_DER;
      correccion += error * KP_LAT;
    }

    // corregir ángulo respecto a pared
    if (paredIzq) {

      float angulo = s3 - s1;
      correccion += angulo * KP_ANG;
    }
